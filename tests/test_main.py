...
async def test_2_request_fails_astro_service():
    ...
    with patch("app.main.astro_client.get_transit_data", new_callable=AsyncMock) as mock_astro:
...
async def test_3_ai_fails_but_data_exists():
    ...
    with patch("app.main.astro_client.get_transit_data", new_callable=AsyncMock) as mock_astro:
        with patch("app.main.ai_engine.generate_consultation", new_callable=AsyncMock) as mock_ai:
